...
class PlayerTest extends FlatSpec with Matchers {
    ...
    val player = PlayerUtil.addShipToPlayer(playerEmpty, ship)
    ...
    it should "say that the specific ship is sunk" in {
        ...
        val playerShipSunk = PlayerUtil.addShipToPlayer(playerEmpty, shipSunk)
    ...
    it should "incremente the score of the player" in {
        val playerScore1 = PlayerUtil.incrementScore(playerEmpty)
    ...
    it should "resset the player" in {
        val playerWithShipScore1 = PlayerUtil.incrementScore(player)
        val playerResset = PlayerUtil.ressetPlayer(playerWithShipScore1)
        (
    ...
    it should "create a new Player with a new ship in its fleet and update his shipsGrid" in {
        ...
        PlayerUtil.addShipToPlayer(playerEmpty, ship) shouldEqual Player("Megmeg", shipsGrid = shipsGrid, fleet = fleet)
    }
    ...
    it should "return the initial player because the ship touched another ship of the payer" in {
        ...
        PlayerUtil.addShipToPlayer(player, ship2) shouldEqual player
    }

    it should "return the initial player because the ship is out of the grid" in {
        PlayerUtil.addShipToPlayer(player, shipOut) shouldEqual player
    }
    ...
    it should "create a new Player without a ship in its fleet" in {
        val shipsGrid = GridUtil.updateMultipleCellsState(emptyGrid, Set((1,1), (1,2), (1,3), (1,4)), CellState.SHIP)
        PlayerUtil.removeShipToPlayer(player, ship) shouldEqual Player("Megmeg", shipsGrid = shipsGrid, fleet = List())
    }
    ...
    it should "create a new player with its shipsGrid updated" in {
        val playerUp = PlayerUtil.updateShipsGrid(player, (1,1), CellState.TOUCH)
    ...
    it should "return the initial player because the cell does NOT belong to the shipsGrid" in {
        val playerUp = PlayerUtil.updateShipsGrid(player, (1,12), CellState.TOUCH)
    ...
    it should "create a new player with its shootsGrid updated" in {
        val playerUp = PlayerUtil.updateShootsGrid(player, (1,1), CellState.TOUCH)
    ...
    it should "return the initial player because the cell does NOT belong to the shootsGrid" in {
        val playerUp = PlayerUtil.updateShootsGrid(player, (1,12), CellState.TOUCH)
        playerUp shouldEqual player
    }
    ...
    it should "create a new player by removing the square of the ship touched" in {
        val playerUp = PlayerUtil.fleetTouched(player, (1,1))
        (
    ...
    it should "create a new player by removing the ship touched because it is sunk" in {
        val player2Ships = PlayerUtil.addShipToPlayer(player, shipSize1)
        val playerUp = PlayerUtil.fleetTouched(player2Ships, (4,4))
        (
    ...
    it should "create a new player with its fleet sunk because we just sunk the last ship" in {
        val playerSmall = PlayerUtil.addShipToPlayer(playerEmpty, shipSize1)
        val playerUp = PlayerUtil.fleetTouched(playerSmall, (4,4))
        (
            playerUp.shipTouched(4,4) shouldEqual None,
    ...
    it should "return the initial player because no ships was touched" in {
        val playerUp = PlayerUtil.fleetTouched(player, (4,4))
        playerUp shouldEqual player
    }
    ...
    it should "update the cell touched by the opponent (MISS)" in {
        val playerTouchedMiss = PlayerUtil.addOpponentShoot(player, (2,2))
    ...
    it should "update the cell touched by the opponent (TOUCH)" in {
        val playerTouchedTouch = PlayerUtil.addOpponentShoot(player, (1,2))
        (
    ...
    it should "return the initial player because the cell was already touched" in {
        val playerTouchedTouch = PlayerUtil.addOpponentShoot(player, (1,2))
        val playerAlreadyTouch = PlayerUtil.addOpponentShoot(player, (1,2))
    ...
    it should "return the initial player because the cell is NOT belong to the shipsGrid" in {
        val playerTouchedOut = PlayerUtil.addOpponentShoot(player, (1,13))
